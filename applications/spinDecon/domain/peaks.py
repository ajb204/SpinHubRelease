...
def peak_sort_key(name):
    """Return a stable residue/peak ordering key.

    Peak labels often contain more than one integer (for example ``A8-N105``).
    The integer represented by the longest run of digits is treated as the
    residue number; ties use the left-most run.  Labels without digits sort
    after numbered labels.
    """
    import re
    ...
    match = max(matches, key=lambda item: (len(item.group(0)), -item.start()))
    return (0, int(match.group(0)), text.lower())
