...
class SliceFrame2D(wx.Panel):
    ...
    def control_box(self):
        ...
        # Slice2D no longer owns peak-list editing. Connectivity/NOE compatibility
        # remains isolated in the explicit 4D bridge below.
    ...
    def _draw_1d_lower_panel(self, peak_index, show_decon=False, show_labels=False):
        ...
        # Historically these came only from conn_data.  A cold-loaded project
        # can legitimately have the authoritative Full peak list in memory
        # without conn_data having been reconstructed, which made report
        # Slice2D exports lose the vertical peak/intensity indicators.  Fall
        # back to the Full-list records so Review and report rendering use the
        # same persistent peak-pick source of truth.
        ...
        if one_d_cb:
            ...
            if(grid_cb):
                ...
                for cn in self.slice_service.connections:
                    ...
                    if(orth_cb and dec_cb):
                        if(cn.v1==sele1):
                            self.axes2.scatter(cn.f3,cn.f2,c='k',s=50,zorder=2,marker='x')
                            # if(sel):
                    elif(orth_cb): #if orthoganol
                        if(cn.v1==sele1):
                            self.axes2.scatter(cn.f3,cn.f2,c='k',s=50,zorder=2,marker='x')
                            # if(sel):
                    elif(dec_cb):
                        if(cn.v1==sele1):
                            self.axes2.scatter(cn.f3,cn.f1,c='k',s=50,zorder=2,marker='x')
                            # if(sel):
                ...
                for sele in self.selection:
                    ...
                    if(orth_cb and dec_cb):
                        if(cn.v1==sele1):
                            self.axes2.scatter(cn.f3,cn.f2,c='r',s=50,zorder=2,marker='x')
                    elif(orth_cb): #if orthoganol
                        if(cn.v1==sele1):
                            self.axes2.scatter(cn.f3,cn.f2,c='r',s=50,zorder=2,marker='x')
                    elif(dec_cb):
                        if(cn.v1==sele1):
                            self.axes2.scatter(cn.f3,cn.f1,c='r',s=50,zorder=2,marker='x')
                    else: #if not orthoganol
                        if(cn.v1==sele2):
                            self.axes2.scatter(cn.f3,cn.f1,c='r',s=50,zorder=2,marker='x')
        ...
        if(grid_cb):
            ...
            for cn in self.slice_service.connections:
                if(orth_cb and dec_cb): #if orthoganol
                    ...
                    if(cn.v1==sele1):
                        self.axes1.scatter(cn.f3,cn.f1,c='k',s=50,zorder=2,marker='x')
                        #if(sel):
            for sele in self.selection:
                cn=self.slice_service.connections[sele]
                if(orth_cb and dec_cb): #if orthoganol
                    if(cn.v1==sele1):
                        self.axes1.scatter(cn.f3,cn.f2,c='r',s=50,zorder=2,marker='x')
                else: #if not orthoganol
                    if(cn.v1==sele1):
                        self.axes1.scatter(cn.f3,cn.f1,c='r',s=50,zorder=2,marker='x')
        ...
        if(self.MAGMA):
            ...
            if(label in list(self.peak_dict.keys())): #if this residue can be translated into magmaPDB language...
                ...
                for ref in list(labs.keys()):
                    ...
                    self.axes1.text(yval,dimH+Width1*0.1,labs[ref],rotation=90,fontsize=8,color='g',va='bottom')
    ...
    def on_pick(self, event):
        if self._handle_full_tool_click(event):
            return
        ...
        if(self.SELECT==1):
            ...
            if(self.sym==True): #do second frame if needed.
                ...
                for i,cn2 in enumerate(self.slice_service.connections):
                    if(cn2.p1==cn.p2):
                        if(cn2.p2==cn.p1):
                            logging.info('Reciprocated:',cn2.tag)
            ...
            self.SELECT=0
            logging.info(self.selection,self.SELECT)
            self.draw_figure()
        if(self.SEARCH==1):
            self.selection=[]
            ...
            findnear_index = self.findnear_index

            if(self.sym==False):
                pk=self.slice_service.peaks[self.ComboBox1.GetSelection()]
                f1=pk.ppmK
                f2=pk.ppmJ
                f3=event.xdata


                i=findnear_index(self.slice_service.axis(0),event.xdata)

                f3=self.slice_service.axis(0)[i]
                #inty=self.slice_service.data[pk.indexI,pk.indexJ,i]
                inty=self.slice_service.data[i,pk.indexJ,pk.indexK]
                logging.info(f1,f2,f3,inty)
                logging.info('Adding Peak:')
                ...
                logging.info(stry)
                test=stry.split()
                cnNew=connEntry(test,sym='n',peak=self.slice_service.peaks,dim=3)
                logging.info('added')
            ...
            #inty=self.slice_service.data[pk.indexI,pk.indexJ,i]
            inty=self.slice_service.data[i,pk.indexJ,pk.indexK]
            logging.info('raw intensity at selection point:',inty)
            ...
            for i,pk in enumerate(self.slice_service.peaks):
                ...
                if(rad2<5E-6): #if selection is less than threshold...
                    logging.info('possible:',pk.name,rad2)
                    poss.append(i)
            # raddy=numpy.array(raddy)
            # maxy=argy[numpy.argmin(raddy)]
            #
            ...
            if(len(poss)==0): #if there are no options, take the smallest distance
                raddy=numpy.array(raddy)
                maxy=argy[numpy.argmin(raddy)]
                poss.append(maxy)
            ...
            #     test=stry.split()
            #     print stry
            #     cnNew=connEntry(test,sym='y',peak=self.slice_service.peaks,dim=3)
            #     print 'added'
            #     self.slice_service.connections.append(cnNew)
            #     self.selection.append(len(self.slice_service.connections)-1)
            #
    ...
    def LEGACY_NOETest(self,refy,maxy):
        logging.info('Testing for NOE:')
        add=0
        ...
        pk2=self.slice_service.peaks[maxy]
        if(pk1.name==pk2.name):
            logging.info('this is a diagonal peak. Not adding')
            add=1

        logging.info(pk1.name,pk2.name)
        ...
        for i,cn in enumerate(self.slice_service.connections):
            if(cn.p1==pk1.name and cn.p2==pk2.name):
                logging.info('NOE already present:',cn.tag)
                nummy+=1
            if(cn.p2==pk1.name and cn.p1==pk2.name):
                logging.info('NOE (reciprocated) already present:',cn.tag)
                nummy+=1
        if(nummy==2):
            logging.info('both peaks are here: not adding.')
            add=1
        i1=self.slice_service.data[pk1.indexI,pk2.indexJ,pk2.indexK]
        i2=self.slice_service.data[pk2.indexI,pk1.indexJ,pk1.indexK]

        logging.info('i1,i2: ',i1,i2)
        ...
        #noise=self.parent.tabOne.dmax*float(self.tabOne.threshBox.GetValue())
        self.noise=float(self.textbox0.GetValue())
        logging.info('noise:',self.noise)
        logging.info('Intensity of selected cross peak:')
        logging.info(i1,i1/self.noise)
        logging.info('Intensity of reciprocated cross peak:')
        logging.info(i2,i2/self.noise)
        if(numpy.fabs(i1)<self.noise):
            logging.info('I1 less than noise. Probably not a cross peak.')
            add=1
        if(numpy.fabs(i2)<self.noise):
            logging.info('I2 less than noise. Probably not a cross peak.')
            add=1
        ...
        logging.info('Ratio (expected to be >50%):')
        logging.info(min(i1,i2)/max(i1,i2)*100.)
        ...
        try: #adjust intensity for deconvolved spectrum
            ...
            pass
        logging.info('intensity1:',i1)
        logging.info('intensity2:',i2)
    ...
    def on_pick(self, event):
        if(self.cb_grid_select.GetValue()==1):
            if(self.pick_cnt==0):
                ...
                for i in range(len(self.slice_service.peaks)):
                    if(float(self.slice_service.peaks[i][1])>float(self.select[0][0]) and float(self.slice_service.peaks[i][1])<float(self.select[1][0])):
                        self.selection.append(i)
    ...
    def LEGACY_AddNOE(self,refy,maxy,i1,i2):
        print('Adding NOE:')
