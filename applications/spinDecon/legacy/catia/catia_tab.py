...
#from .Frames.deconFrame import deconFrame
from .catia_app import catiaFrame

#from .Frames.slicePlot4D import SliceFrame4D
#from .Frames.slicePlot2D import SliceFrame2D
# from .Frames.slicePlot2Dnorm import SliceFrame2Dnorm
#from .Frames.slicePlot import SliceFrame
# from .Frames.NOEframe import NOEFrame
#from .Frames.Projection import Projection

#from .Frames.magmaFrame import magmaFrame
#from .Frames.Full3D import Full3D
#from .Frames.OneDView import OneDFrame
#from .Frames.Pseudo2D import Pseudo2D
#from .Frames.Pseudo3D import Pseudo3D
#from .Frames.STD_frame import STDFrame
#from .Frames.PhasingSpectra import Phasing
...
class NotebookDemo(wx.Notebook):
    ...
    """
    def __init__(self, parent,panel,catiaParFile):
        ...
        self.catiaParFile=catiaParFile

        self.tabOne = catiaFrame(self,self.catiaParFile)
        self.AddPage(self.tabOne, "CATIA")

    """
    def AddMagmaTab(self, event,tabOne):
        self.KillPage('Magma')
        self.magmaTab = magmaFrame(self, self.deconParFile)
        self.AddPage(self.magmaTab, "Magma")

    def AddTabPseudo2D(self,event,tabOne):
        #try:
        self.KillPage('Pseudo2D')
        self.tabPseudo = Pseudo2D(self,self.tabOne)
        self.tabPseudo.Bind(wx.EVT_SET_FOCUS, self.tabPseudo.onFocus)
        self.AddPage(self.tabPseudo, "Pseudo2D")

        #except:
        #    print 'Failed making 2D planes'
        pass

    def AddTabSTD(self,event,tabOne):
        #try:
        self.KillPage('uSTA')
        self.tabSTD = STDFrame(self,self.tabOne)
        self.tabSTD.Bind(wx.EVT_SET_FOCUS, self.tabSTD.onFocus)
        self.AddPage(self.tabSTD, "uSTA")

        #except:
        #    print 'Failed making 2D planes'
        pass

    def AddTabPseudo3D(self,event,tabOne):
        #try:
        self.KillPage('Fitting')
        self.tabPseudo = Pseudo3D(self,self.tabOne)
        self.tabPseudo.Bind(wx.EVT_SET_FOCUS, self.tabPseudo.onFocus)
        self.AddPage(self.tabPseudo, "Fitting")

        #except:
        #    print 'Failed making 2D planes'
        pass

    def AddTabPhasing(self,event,tabOne):
        #try:
        self.KillPage('Phasing')
        self.tabPhasing = Phasing(self,self.tabOne)
        self.tabPhasing.Bind(wx.EVT_SET_FOCUS, self.tabPhasing.onFocus)
        self.AddPage(self.tabPhasing, "Phasing")

    def AddTabTwo(self,event,tabOne):
        #try:
        self.KillPage('Projections')
        self.tabTwo = Projection(self,self.tabOne)
        self.tabTwo.Bind(wx.EVT_SET_FOCUS, self.tabTwo.onFocus)

        self.AddPage(self.tabTwo, "Projections")
    ...
    def AddTabThree(self,event,tabOne):
        #try:
        self.KillPage('1D Slices')
        self.tabThree = SliceFrame(self,self.tabOne)
        ##tabOne.SetBackgroundColour("Gray")
        self.AddPage(self.tabThree, "1D Slices")
        pass

    def AddTabFour(self,event,tabOne):

        self.KillPage('2D Slices')
        self.tabFour = SliceFrame2D(self,self.tabOne)
        self.AddPage(self.tabFour,"2D Slices")
        pass

    def AddTabFive(self, event, tabOne):

        self.KillPage('Full 3D')
        self.tabFive = Full3D(self,self.tabOne)
        self.AddPage(self.tabFive,"Full 3D")
        pass

    def AddTab1D(self, event, tabOne):
        self.KillPage('1D view')
        self.tab1D = OneDFrame(self,self.tabOne)
        self.AddPage(self.tab1D,"1D view")
        pass
...
class MyApp(wx.Frame):
    ...
    #----------------------------------------------------------------------
    def __init__(self,catiaParFile):
        """Constructor"""
        self.monitorWidth, self.monitorHeight = wx.GetDisplaySize()
        wx.Frame.__init__(self, None, wx.ID_ANY,
                          "catiaNMR - "+os.getcwd().split('/')[-1], wx.DefaultPosition,
        ...
        self.notebook = NotebookDemo(self,panel,catiaParFile)
        ...
        if(os.path.exists(catiaParFile)==1):
            self.TestPath(catiaParFile)
            self.DoLoad(catiaParFile)
    ...
    def OnNew(self,event):
        ...
        dlg = wx.FileDialog(
            ...
            defaultFile='catiaParFile',
            wildcard=file_choices,
            style=wx.FD_SAVE)
        if dlg.ShowModal() == wx.ID_OK:
            path = dlg.GetPath()
            self.catiaParFile=path

            outy=open(self.catiaParFile,'w');outy.close()
    ...
    def OnLoadResults(self, event):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            self.DoLoad(path)
    def TestPath(self,catiaParFile):
        Parse=self.notebook.tabOne.Parse #messy...

        self.deconParFile=catiaParFile
        indir=Parse(self.catiaParFile,'indir')
        fiddir=Parse(self.catiaParFile,'fiddir')
    ...
    def DoLoad(self,path):
        #load magmaTab
        self.notebook.catiaParFile=path
...
if __name__ == "__main__":
    #print sys.argv
    if(len(sys.argv)==2):
        catiaParFile=sys.argv[1]
    else:
        catiaParFile='catiaParFile'

    if(os.path.exists(catiaParFile)==0):
        outy=open('catiaParFile','w');outy.close()

    app = wx.App()
    frame = MyApp(catiaParFile)
    app.MainLoop()
