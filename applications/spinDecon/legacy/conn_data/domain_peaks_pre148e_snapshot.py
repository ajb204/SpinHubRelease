...
"""
#storage class for peaks
class peakEntry():
    def __init__(self,test):
        self.name=test[0]
        self.y=float(test[1])
        self.x=float(test[2])
        # Extensible measurements/classifications attached by analysis tools.
        self.analysis={}



        
#storage class for cross links
class connEntry():
    def __init__(self,test,sym='y',peak=[],dim=3):

        self.sym=sym
        if(sym=='y'):
            cnt=0
            self.p1=test[cnt];cnt+=1
            self.p2=test[cnt];cnt+=1
            self.f1=float(test[cnt]);cnt+=1
            self.f2=float(test[cnt]);cnt+=1
            self.f3=float(test[cnt]);cnt+=1
            if(dim==4):
                self.f4=float(test[cnt]);cnt+=1
            self.s1=float(test[cnt]);cnt+=1
            self.s2=float(test[cnt]);cnt+=1
            self.n1=float(test[cnt]);cnt+=1
            self.n2=float(test[cnt]);cnt+=1
            try:
                self.frac=float(test[cnt]);cnt+=1
                self.distScore=float(test[cnt]);cnt+=1
                self.distppm=float(test[cnt]);cnt+=1
                self.Intscore=float(test[cnt]);cnt+=1
            except:
                pass

            try:
                self.hm=float(test[cnt]);cnt+=1
                self.cm=float(test[cnt]);cnt+=1
            except:
                self.hm=0
                self.cm=0
                pass

            self.v1=self.GetInd(self.p1,peak)
            self.v2=self.GetInd(self.p2,peak)

            if(self.v1!=-1 and self.v2!=-1):
                self.tag=peak[self.v1].name+'-'+peak[self.v2].name
                self.tag2=peak[self.v2].name
            else:
                self.tag='xxx'
                self.tag2='xxx'
            ...
            if dim ==1:
                self.p1=test[0]
                self.p2=test[0]
                self.f1=float(test[2])
                self.s1=float(test[3])
            elif dim == 2:
                self.p1=test[0]
                ...
                self.s1=float(test[4])

            self.v1=self.GetInd(self.p1.split('_')[0],peak)
            self.v2=self.v1
            if(self.v1!=-1):
                self.tag=peak[self.v1].name
                self.tag2=peak[self.v1].name
            else:
                self.tag='xxx'
                self.tag2='xxx'
            self.tp=''
    ...
    def GetInd(self,p1,peak):
        for v1,pe in enumerate(peak):
            #print pe,p1
            if(pe.name==p1):
                return v1
