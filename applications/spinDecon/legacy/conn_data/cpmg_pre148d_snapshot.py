...
class CPMGFrame(wx.Frame):
    ...
    def __init__(self,parent,id,title,inherit,pth):
        ...
        wx.Frame.__init__(self, id=wxID_FRAME1, name='', parent=parent,
              ...
              style=wx.DEFAULT_FRAME_STYLE, title='CPMG Frame')
        ...
        if(pth==''):
            ...
            self.raw=self.pth+'/raw'
        self.fuda_dir = inherit._fuda_dir()
            
        self.savefile=self.raw+'/catia/frame.save'
        if(os.path.exists(self.savefile)==0):
            self.PathExists((self.raw+'/catia',))
        ...
        self.buttonGuess = wx.Button(self, label="Guess")
        ...
        self.vboxOpts.Add(self.datasets)


        #self.PackOpts(self.fieldboxtxt,self.fieldbox)
        #self.PackOpts(self.tempboxtxt,self.tempbox)

        self.PackOpts(self.basistxt,self.basisCombo)
        self.PackOpts(self.seqfiltxt,self.seqfilCombo)
        self.PackOpts(self.RexScreenBoxTxt,self.RexScreenBox)

        self.PackOpts(self.TimeT2BoxTxt,self.TimeT2Box)
        axis_row = wx.BoxSizer(wx.HORIZONTAL)
        axis_row.Add(self.pseudoAxisTxt, 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 6)
        axis_row.Add(self.pseudoAxisCombo, 1, wx.EXPAND | wx.RIGHT, 4)
        axis_row.Add(self.pseudoAxisOpen, 0)
        self.vboxOpts.Add(axis_row, 0, wx.EXPAND | wx.ALL, 3)
        
        #self.vboxOpts.Add(setboxS)
        #self.vboxOpts.Add(locboxS)
        
        self.vboxOpts.Add(self.buttonRefresh)

        
        self.vboxOpts.Add(self.buttonClose)
        self.vboxOpts.Add(self.buttonPeakConvert)
        self.vboxOpts.Add(self.buttonRun)
        self.vboxOpts.Add(self.buttonSave)
        self.vboxOpts.Add(self.buttonLoad)
        self.vboxOpts.Add(self.buttonWipe)
        self.vboxOpts.Add(self.buttonGuess)

        


        #self.peakCombo=wx.ComboBox(self, -1, size=(80, -1),  style=wx.CB_READONLY)
        ...
        self.fullSizer.Add(self.vboxOpts)
        self.create_main_panel() #add graph panel

        #self.fullSizer.Add(self.vbox)
        #self.fullSizer.Add(fitboxS)
        self.sizerMain.Add(self.fullSizer)

        self.sizerMain.Add(self.listbox)
        #self.sizerBut= wx.BoxSizer(wx.HORIZONTAL)

        #self.sizerBut.Add(self.buttonAddDataset)
        #self.sizerBut.Add(self.buttonRemDataset)
        #self.sizerBut.Add(self.buttonSave)
        #self.sizerBut.Add(self.buttonClose)

        #self.sizerBut.Add(self.peakCombo)
        #self.sizerBut.Add(self.buttonRun)
        #self.sizerBut.Add(self.buttonPeakConvert)
        
        #self.sizerMain.Add(self.sizerBut)
    ...
    def cpmg_Baldwin(self):
        ...
        dw=self.dw
        Trel=self.Time_T2
        ncyc=Trel*self.cpmgX  #set by reading in FUDA file
        ...
        g3=numpy.cos(0.5*numpy.arctan2(g1,g2))*(g1**2.0+g2**2.0)**(1/4.0)   #trig faster than square roots
        g4=numpy.sin(0.5*numpy.arctan2(g1,g2))*(g1**2.0+g2**2.0)**(1/4.0)   #trig faster than square roots
        #########################################################################
    ...
    def DoFit(self,pk):
        ...
        self.kex=1000
        self.dw=1.*self.dfrq
        self.R0=numpy.min(self.cpmgY)
        
        x0=leastsq(self.ChiCPMG,self.PackCPMG())
        print (pk,x0)

        self.chi2Local=numpy.average(self.ChiCPMG(self.PackCPMG())**2.)
        ...
        for i,c1 in enumerate(col1):
            if(c1==pk):
                Rex=numpy.max(self.cpmgF)-numpy.min(self.cpmgF)
                self.datasets.SetStringItem(i,2,'%.2f' % Rex)
                self.datasets.SetStringItem(i,3,'%.2f' % (1.-self.chi2Local/self.chi2Line))
                #self.datasets.SetStringItem(i,4,'%.2f' % self.chi2Local)


                
                if(Rex<self.RexScreen or Rex!=Rex):
                    self.datasets.SetStringItem(i,1,str(False))
                else:
                    self.datasets.SetStringItem(i,1,str(True))


                
        if(pk not in self.cpmgLocal.keys()):
            self.cpmgLocal[pk]={}
        self.cpmgLocal[pk]['R0line']=self.R0line
        self.cpmgLocal[pk]['chi2Line']=self.chi2Line
        self.cpmgLocal[pk]['dw']=self.dw/self.dfrq
        self.cpmgLocal[pk]['pb']=self.pb
        self.cpmgLocal[pk]['kex']=self.kex
        self.cpmgLocal[pk]['R0']=self.R0
    ...
    def OnClick(self,event): #when selecting a peak...plot what we have.
        ...
        if(col2=='True'):
            self.datasets.SetStringItem(sele,1,str(False))
        else:
            self.datasets.SetStringItem(sele,1,str(True))
    ...
    def OnButtonRefresh(self,event):
        ...
        self.datasets.InsertColumn(1, 'Include', width = 80,format=wx.LIST_FORMAT_CENTRE)

        self.datasets.InsertColumn(2, 'Rex', width = 80,format=wx.LIST_FORMAT_CENTRE)
        self.datasets.InsertColumn(3, 'Chi2diff', width = 80,format=wx.LIST_FORMAT_CENTRE)
        ...
        for peak in self.peaks:
            cnt+=1
            self.datasets.InsertStringItem(num_items,str(cnt))
            self.datasets.SetStringItem(num_items,0,peak)
            self.datasets.SetStringItem(num_items,1,str(True))
        pass
    ...
    def OnButtonSort(self,event):
        ...
        for i,arg in enumerate(s):
            ...
            self.datasets.SetStringItem(i,0,str(col1[arg]))
            self.datasets.SetStringItem(i,1,str(col2[arg]))
            self.datasets.SetStringItem(i,2,str(col3[arg]))
    ...
    def MakeCPMGcurve(self,pk,infile):
        ...
        nucpmg=self.Get_nuCPMG()
        print ('NuCPMG:',nucpmg)
        if(nucpmg[0]==-1):
            return
            
        nucpmg=numpy.array(nucpmg)
        ...
        print ('Non-zero NuCPMG:',nucpmgActual)
        #field=d[0]/TIME_T2
        ...
        if len(data_lines) != len(nucpmg):
            inputfile.close()
            wx.MessageBox('Pseudo-axis has %d values; fitted data contain %d planes' % (len(nucpmg), len(data_lines)),
                          'Pseudo-axis', wx.OK | wx.ICON_WARNING)
            return
        Data=[]
        dublicate=[]
        dubFreq=[]
        ...
        for line in lines:
            ...
            if not ( line[0] == "#"):
                ...
                if(self.typ=='bruk'):
                    ...
                    if ( math.fabs(nucpmg[j])<1e-6):

                        Ref=[0,float(temp[1]),float(temp[2])]
                    else:
                        Data.append([nucpmg[j],float(temp[1]),float(temp[2])])
                        print(temp)
                j+=1    
                for d in range(len(Data)-1):
                    if ( math.fabs(float(temp[0])-Data[d][0]) < 1e-6 ):
                        dublicate.append(math.pow(float(temp[1])-Data[d][1],2.))
                        dubFreq.append(float(temp[0]))
        ...
        try:
            ...
            Ref = Data[0]
        #
        # Get dublicate data (Flemming's spelling)
        if(len(dublicate)==0):
            StdErr=0.3
        else:
            StdErr=0.
        ...
        ofn=open(infile+".cpmg",'w')
        ofn.write("#%11s%15s%13s\n" % ('nu_cpmg(Hz)','R2(1/s)','Esd(R2)'))
        #print len(Data)
        #sys.exit(100)
        print('Reference intensity:',Ref)
        print('Duplicated frequency:',dubFreq)
        print(len(Data))
    ...
    def draw_figure(self,event):
        ...
        self.ax.errorbar(self.cpmgX,self.cpmgY,yerr=self.cpmgE,fmt='o')                
