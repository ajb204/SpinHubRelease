...
class DataStore:
    ...
    The store keeps the currently loaded spectrum, derived projections,
    deconvolved data, and peak/connection lists together so that tabs can
    observe and edit the same objects.
    """
    ...
    # Legacy connectivity payload retained only for old NOE/4D compatibility.
    # Canonical peaks live in the Full Peak List; new connection work must use
    # a dedicated connection model referencing those peak identities.
    conn_data: list[Any] = field(default_factory=list)
    spectrumfile: str = ""
    STD_raw_path: str = ""
    STD_std_path: str = ""
    STD: Any = None
