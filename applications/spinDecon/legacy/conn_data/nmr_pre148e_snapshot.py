...
from spinDecon.domain.peaks import peakEntry, connEntry, diagEntry
...
class NMRWorkspace(wx.Panel):
    ...
    _DATASTORE_FIELDS = {
        ...
        "dmax", "noiseVal", "projectedData", "datadec", "dicdec", "peak", "conn_data",
    ...
    def _update_full_peak_controls(self):
        ...
        for name in ('referencePeakLab', 'referencePeakBox', 'openPeakFileBtn',
                     'buttonPeaky', 'buttonReadPeak', 'buttonReferencePeakList'):
            control = getattr(self, name, None)
            if control is not None:
                control.Show(not pseudo2d)
    ...
    def _rebuild_projected_peak_lists(self):
        ...
        # True 2D spectra use the same authoritative reference peak list as
        # the main NMR display.  Publish that list under the canonical
        # Pseudo3D/fitting display axes as well.  Previously this routine
        # returned before creating any projected lists for dim==2, so the
        # Fitting tab's Peaks checkbox had no points to draw.
        if int(getattr(self, 'dim', 0) or 0) == 2:
            ...
            if pseudo_view is not None:
                ...
                points = []
                for index, peak in enumerate(self.get_reference_peaks()):
                    points.append({
                        'x': float(peak.x),
                        'y': float(peak.y),
    ...
    def _load_decon_outputs(self, infile, load_peaks=True):
        ...
        if spectral_dim_count == 3 and not topology.has_pseudo_axis:
            ...
            if load_peaks and getattr(self, "store", None) is not None:
                self.store.save_peak_list("decon", peak=self.peak, conn_data=self.conn_data, pkIdx=self.pkIdx, pkSlice1Ddec=self.pkSlice1Ddec, noeTags=self.noeTags)
            return True
    ...
    def GetConn(self,infile,sym='y'):
        ...
        for line in inny.readlines():
            test=line.split()
            # Peak/connection dimensionality is spectral.  The real pseudo
            # axis identifies a series plane and is never a peak coordinate.
            self.conn_data.append(connEntry(test, sym=sym, peak=self.peak, dim=self.dim))
        ...
        self.Status()
        # ``tabFour`` is the legacy connection-data viewer.  Modern workflow
        # configurations do not necessarily create that notebook page, so
        # connection parsing must never depend on it being present.  Publish
        # to the viewer only when the optional page actually exists.
        if self.dim == 3:
            tab_four = getattr(self.parent, 'tabFour', None)
            if tab_four is not None:
                try:
                    if hasattr(self.parent, 'PageExists') and not self.parent.PageExists('Connections'):
                        tab_four = None
                except Exception:
                    # PageExists is only a presentation hint; capability of
                    # the page object itself is sufficient here.
                    pass
            if tab_four is not None:
                try:
                    update = getattr(tab_four, 'update_conn_data', None)
                    if callable(update):
                        update()
                    ...
                    print('Legacy connection view refresh skipped:', exc)
    ...
    def OnButtonPeaky(self,event):  #load/focus the single peak frame
        # PeakFrame is a project-level editor for the shared Reference 2D list.
    ...
    def _finish_decon_run(self, run_dim, run_dimProj, run_dec3d, run_ncpus, decset, rc=0, caller='main', expected_peak_path=None, projection_labels=None):
        ...
        if caller == 'main' and run_dimProj == False:  # main-window nD decon
            ...
            self.corrFile = peak_path
            self.OnButtonAnalyse(True)
            # The legacy connection viewer is optional.  Analysis completion
            # and workflow state are driven by the generated nD peak list, not
            # by whether this deprecated notebook page is installed.
            if topology.spectral_dim_count > 2:
                tab_four = getattr(self.parent, 'tabFour', None)
                if tab_four is not None:
                    try:
                        tab_four.conn_data = self.conn_data
                    except Exception as exc:
                        print('Legacy connection data publication skipped:', exc)
        else:
    ...
    def OnButtonAnalyse(self,event):
        ...
        if spectral_dim_count <= 2:
            sym = 'n'
        elif(self.cb_grid.IsChecked()==True):
            sym='y'
        else:
            sym='n'

        # A pseudo2D experiment is a series of 1D spectra.  Its deconvolution
        # product belongs to the stored 1D projection and has no multidimensional
        # connectivity records.  Feeding a Full 1D list to GetConn/connEntry
        # incorrectly assumes four-or-more connection columns and raises an
        # IndexError.  The projection loader above has already populated the
        # deconvolved 1D data, so deliberately skip connectivity parsing here.
        if spectral_dim_count == 1 and topology.has_pseudo_axis:
            # Publish the associated 1D projection peak list without trying to
            # interpret it as a multidimensional connection file.
            self._load_pseudo2d_projection_peaks(self.corrFile)
        else:
            self.GetConn(self.corrFile,sym=sym)
        if getattr(self, "store", None) is not None:
            self.store.save_peak_list("decon", peak=self.peak, conn_data=self.conn_data, pkIdx=self.pkIdx, pkSlice1D=self.pkSlice1D, pkSlice1Ddec=self.pkSlice1Ddec, Grps=self.Grps, noeTags=self.noeTags)
    ...
    def ReadPeakListFile(self):  #master read-in function for reference peaklists.
        ...
        if spectral_dim_count == 2:  #if 2 dimensional.
            ...
            for pk in self.peak:
                self.conn_data.append(connEntry([pk.name, pk.x, pk.y],sym='n',peak=self.peak,dim=spectral_dim_count))
            #print(self.peak)
