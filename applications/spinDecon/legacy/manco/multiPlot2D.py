...
#        self.hbox.Add(self.slider_width, 0, border=3, flag=flags)
    ...
    def GetSlice2d(self,spectrumfile):
        ...
        self.index0.append(index0)
        self.index1.append(index1)
        self.index2.append(index2)
    ...
    #get 2d strips from 3d data
    def ReSlice2dNOE(self,i1,i2,peak,width,sp):
        ...
        ptC2=self.findnear_index(float(peak[i2][2]),self.index0[sp])#find the nearest point to desired chemical shift in carbon index
        ...
        ptC2_min=self.findnear_index(float(peak[i2][2])-float(width)/2,self.index0[sp])#find the nearest point to desired chemical shift in carbon index
        ptC2_max=self.findnear_index(float(peak[i2][2])+float(width)/2,self.index0[sp])#find the nearest point to desired chemical shift in carbon index


        slice2d=self.data[sp][ptC2_max:ptC2_min,ptC_max:ptC_min,ptH] #extract the relevant 2d slice

    
        Xs=[]
        Ys=[]
        Zs=[]
        for e in range(len(slice2d[1])):  #proton
            xs=[]
            ys=[]
            zs=[]
            for d in range(len(slice2d)): #carbon
                xs.append(self.index1[sp][ptC_max+d])
                ys.append(self.index1[sp][ptC2_max+e])
                zs.append(slice2d[d][e])
            Xs.append(xs)
            Ys.append(ys)
            Zs.append(zs)
    ...
    def draw_figure(self):
        ...
        if(1==0):
            ...
            self.axes.clear()
            Xs,Ys,Zs=self.ReSlice2dNOE(self.ComboBox1.GetSelection(),self.ComboBox2.GetSelection(),self.peak,self.width,0)
            ...
            self.axes.contour(Ys, Xs, Zs,levels,cmap=colormap) #plot pdb network
            ...
            xl.append((float(self.peak[self.ComboBox1.GetSelection()][2])))
            hl=[]
            hl.append((float(self.peak[self.ComboBox2.GetSelection()][2])+self.width/2))
            hl.append((float(self.peak[self.ComboBox2.GetSelection()][2])-self.width/2))
            ...
            xl.append((float(self.peak[self.ComboBox2.GetSelection()][2])))
            hl=[]
            hl.append((float(self.peak[self.ComboBox1.GetSelection()][2])+self.width/2))
            hl.append((float(self.peak[self.ComboBox1.GetSelection()][2])-self.width/2))
            ...
            levels=numpy.array(levels)
            levels=numpy.concatenate((-1*levels[::-1],levels))
            self.axes.set_title(self.ComboBox2.GetValue()+" to "+self.ComboBox1.GetValue(),fontsize=8)

            for i in range(len(self.spectrumfile)):
                Xs,Ys,Zs=self.ReSlice2dNOE(self.ComboBox2.GetSelection(),self.ComboBox1.GetSelection(),self.peak,self.width)
                self.axes.contour(Ys, Xs, Zs,levels,cmap=colormap) #plot pdb network
        ...
        for i in range(len(self.spectrumfile)):
            Xs,Ys,Zs=self.ReSlice2d(self.inc2,self.ComboBox2.GetSelection(),self.peak,self.width,i)
        ...
        if(self.cb_grid_auto.GetValue()==1):#if auto-detect has been run...
            for j in range(len(self.conn_data)):#search through the cross peaks
                if(self.index_data[self.conn_data[j][0]]==self.ComboBox2.GetValue()):#find references for the current plane...
                    for k in range(len(self.peak)):#look through the peak list...
                        if(self.peak[k][0]==self.index_data[self.conn_data[j][1]]):#to find the reference for the reciprocated cross peak...
                            self.axes.scatter(float(self.peak[k][2]),float(self.peak[self.ComboBox2.GetSelection()][1]),c='k',s=50)

                            #if(len(self.selection)==0):#if there is no selection, print all peak labels
                            self.axes.text(float(self.peak[k][2]),float(self.peak[self.ComboBox2.GetSelection()][1]),self.index_data[self.conn_data[j][1]])
                            #else:
                            ...
                            #            self.axes.text(float(self.peak[k][2]),float(self.peak[self.ComboBox2.GetSelection()][1]),self.index_data[self.conn_data[j][1]],color='r')
        y_max2=Ys[0][0]
        ...
        if(1==1): #plot the NOE line
            ...
            xl.append((float(self.peak[self.ComboBox2.GetSelection()][2])))
            hl=[]
            hl.append((float(self.peak[self.ComboBox2.GetSelection()][1])))
            hl.append((float(self.peak[self.ComboBox2.GetSelection()][1])))
            self.axes.plot(xl,hl,'green') #horizontal
            yd=[] 
            yd.append(y_max2)
            yd.append((float(self.peak[self.ComboBox2.GetSelection()][1])))
            xd=[]
        ...
        if(self.ax_reset2==1): #if we want to reset the axis
            self.axes.set_xlim(x_max2,x_min2)
            self.axes.set_ylim(y_min2,y_max2)
            self.xmin2=x_min2
            self.xmax2=x_max2
            self.ymin2=y_min2
            self.ymax2=y_max2
        else:#otherwise use the last saved values       
            self.axes.set_xlim(self.xmin2,self.xmax2)
            self.axes.set_ylim(self.ymin2,self.ymax2)
        
        self.ax_reset2=0 #after a reset assume we have no further need to reset axes


        #self.axes.set_xlabel(self.parent.tabOne.labb[0]+'(ppm)',fontsize=8)
        #self.axes.set_ylabel(self.parent.tabOne.labb[2]+'(ppm)',fontsize=8)


        self.xmin2,self.xmax2=self.axes.get_xlim()
        self.ymin2,self.ymax2=self.axes.get_ylim()
        ...
        Xs,Ys,Zs=self.ReSlice2d(self.inc,self.ComboBox1.GetSelection(),self.peak,self.width,0)
        max_level=float(self.textbox1.GetValue())#set contour max level from box
        min_level=float(self.textbox0.GetValue())#set contour min level from box
        ctr_level=int(self.textbox2.GetValue())  #set the cnumber of contours from box
        #levels=[]                                #calculate the contour levels
    ...
    def _init_ctrls(self,prnt,parent):
        ...
        for line in inny.readlines():
            ...
            if(len(test)>0):
                ...
                add.append(int(test[1]))

                conf=1
                if(float(test[6])<0.5): #distance between cross peaks
                    conf+=2
                if(float(test[6])<0.1):
                    conf+=3
                if(float(parent.tabOne.conn_data[cnt-1][2])>0.03):
                    conf+=1
                if(float(parent.tabOne.conn_data[cnt-1][2])>0.06):
                    conf+=2
        ...
        self.lc.InsertColumn(4, 's/n 1')
        self.lc.InsertColumn(5, 's/n 2')
        self.lc.InsertColumn(6, 'diff')
        self.lc.InsertColumn(7, 'IntDist')
        self.lc.InsertColumn(8, 'Shift')
        ...
        for key,data in items:
            ...
            self.lc.SetStringItem(num_items, 1,str(parent.peak[data[1]][0]))  #add atom            

            #self.lc.SetStringItem(num_items, 0,str(data[0]))  #add atom        
            #self.lc.SetStringItem(num_items, 1,str(data[1]))  #add atom            
            for i in range(len(data)-2):
                self.lc.SetStringItem(num_items, i+2,str(data[i+2]))  #add atom
