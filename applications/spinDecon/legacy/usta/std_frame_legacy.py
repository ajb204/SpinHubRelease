...
class STDFrame(wx.Panel):
    ...
    def drawing_box(self):
        ...
        self.drawbutton = wx.Button(self.vbox2Lbl, -1, "Draw!", size=(-1,22))

        self.cb_grid = wx.CheckBox(self.vbox2Lbl, -1,"Peaks",style=wx.ALIGN_RIGHT)
        self.cb_calc = wx.CheckBox(self.vbox2Lbl, -1,"ShowCalc",style=wx.ALIGN_RIGHT)
    ...
    def phasing_box(self):
        ...
        self.p0_slider = wx.Slider(self.vbox3Lbl, value = 0, minValue = -360, maxValue = 360,size=(150,-1),
        style = wx.SL_HORIZONTAL| wx.SL_AUTOTICKS)
        self.p0_slider.Bind(wx.EVT_SLIDER, self.on_p0)
        self.p1_slider = wx.Slider(self.vbox3Lbl, value = 0, minValue = -360, maxValue = 360,size=(150,-1),
        style = wx.SL_HORIZONTAL| wx.SL_AUTOTICKS)
        self.p1_slider.Bind(wx.EVT_SLIDER, self.on_p0)

        self.p0Lab = wx.StaticText(self.vbox3Lbl, label="P0:")
        self.p1Lab = wx.StaticText(self.vbox3Lbl, label="P1:")
        self.p0 = wx.TextCtrl(self.vbox3Lbl,size=(50,22) )
        self.p1 = wx.TextCtrl(self.vbox3Lbl,size=(50,22) )
        self.p0.SetValue('0')
        self.p1.SetValue('0')
    ...
    def Projections_box(self):
        ...
        self.cb_1D = wx.CheckBox(self.vbox4Lbl, -1,"1D",style=wx.ALIGN_RIGHT)
        self.cb_STD = wx.CheckBox(self.vbox4Lbl, -1,"STD",style=wx.ALIGN_RIGHT)
    ...
    def slices_box(self):
        ...
        self.upSliceButton = wx.Button(self.vboxSlicesLbl, size = (30,22),label="+",style=wx.ALIGN_RIGHT)
        self.downSliceButton = wx.Button(self.vboxSlicesLbl,size= (30,22),label="-",style=wx.ALIGN_RIGHT)
        self.current_slice_box = wx.TextCtrl(self.vboxSlicesLbl,size=(20,22))
    ...
    def OnButtonDecon(self,event):
        ...
        if service is None:
            from spinDecon.decon_service import DeconService
    ...
    def OnButtonAnalyse(self, event):
        if os.path.exists('out/raw_prot.3') and os.path.exists('out/std_prot.3') and os.path.exists(self.parent.tabOne.STD_raw_path+'.decon') and os.path.exists(self.parent.tabOne.STD_std_path+'.decon'):
            ...
            self.raw_dic_dec,self.raw_data_dec=ng.pipe.read(self.parent.tabOne.STD_raw_path+'.decon')
            self.conn_data=[]
            ...
            self.raw_dic_dec,self.raw_data_dec=ng.pipe.read(self.parent.tabOne.STD_std_path+'.decon')
            self.conn_data_STD=[]
    ...
    def save(self):
        ...
        target_path = os.path.join(self.parent.tabOne.dirBox.GetValue(), self.parent.tabOne.deconParFile)
        print('saving:', target_path)
        update_parameter_file(target_path, write, source_path=self.parent.tabOne.deconParFile)
    ...
    def OnButtonPeakFit(self, event):
        from . import peakFitFrame
        import importib
        peakFitFrame=importlib.reload(peakFitFrame)
        bool = peakFitFrame.peakFitFrame(self.parent.tabOne)
        peakFitFrame.peakFitFrame.Bind(self, wx.EVT_CLOSE, self.update_sigs)
