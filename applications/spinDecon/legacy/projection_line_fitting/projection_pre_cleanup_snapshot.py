...
class Projection(wx.Panel):
    ...
    def peak_fit(self, event):
        ...
        peak_points, peak_labels = self._peak_points_for_overlay(full_peaks)
        for (loc1, loc2), lab in zip(peak_points, peak_labels):
            index1 = self.uc0.f(str(float(loc1))+' ppm')
            index2 = self.uc1.f(str(float(loc2))+' ppm')
            match = re.findall(r'[0-9]+', str(lab))
            peak_id = int(match[0]) if match else len(self.peak_list)
            self.peak_list.append([index1, index2, float(loc1), float(loc2), peak_id])
            self.peak_list_names.append(str(lab))
