...
class Projection(wx.Panel):
    ...
    def focus_2d_peak(self, peak, width_fraction=0.10):
        """Select and zoom the physical 2D Projection view around a peak."""
        if not self._is_true_2d_spectrum() or peak is None:
            return False
        try:
            x, y = float(peak.x), float(peak.y)
            self._selected_2d_peak_xy = (x, y)
            # Selection ornament mirrors peakFrame's persistent selection style.
            # Rebuild just this transient artist without forcing a contour redraw.
            old_artist = getattr(self, '_selected_2d_peak_artist', None)
            if old_artist is not None:
                try:
                    old_artist.remove()
                except (ValueError, RuntimeError):
                    pass
            self._selected_2d_peak_artist = self.axes.scatter(
                x, y, c=[[0.3, 0.5, 0.0, 1.0]], s=140, marker='x',
                zorder=21, linewidth=1.5)
            xs = numpy.asarray(self.uc1.ppms_scale, dtype=float)
            ys = numpy.asarray(self.uc0.ppms_scale, dtype=float)
            half_x = abs(float(numpy.nanmax(xs)) - float(numpy.nanmin(xs))) * float(width_fraction) / 2.0
            half_y = abs(float(numpy.nanmax(ys)) - float(numpy.nanmin(ys))) * float(width_fraction) / 2.0
            self.axes.set_xlim(x - half_x, x + half_x)
            self.axes.set_ylim(y - half_y, y + half_y)
            self.last_mouse_x, self.last_mouse_y = x, y
            self._update_2d_traces(x, y, redraw=False)
            self.canvas.draw_idle()
            return True
    ...
    def _peak_points_for_overlay(self, entries, swap=False):
        ...
        for entry in entries or []:
            if hasattr(entry, "f1") and hasattr(entry, "f2"):
                ...
                # Full Peak List records are dictionary payloads after the
                # datastore refactor.  Their coordinates remain in peak-list
                # f1..fN order; for the physical-2D Projection display f1 is X
                # (direct) and f2 is Y (indirect), hence the existing swap=True
                # call below maps them onto the plotted axes.
                coords = entry.get("coordinates") or ()
                if len(coords) < 2:
                    continue
                x = float(coords[1] if swap else coords[0])
                y = float(coords[0] if swap else coords[1])
    ...
    def draw_2d(self, keepaxes=False):
        ...
        self.axes.clear()
        self._selected_2d_peak_artist = None
        ...
        # The toolbar Peaks state is persistent across redraws.  Draw the
        # authoritative Full Peak List as part of the normal true-2D render,
        # rather than as a one-off toolbar side effect (which was immediately
        # lost on the next contour redraw).
        self.peaks_text = []
        self.peaks_scatter = []
        if self.cb_grid.IsChecked():
            full_peaks = self._cached_peak_overlay()
            self.peak_list_locs, peak_labels = self._peak_points_for_overlay(full_peaks, swap=True)
            if len(self.peak_list_locs) > 0:
                for (loc1, loc2), lab in zip(self.peak_list_locs, peak_labels):
                    if lab:
                        self.peaks_text.append(
                            self.axes.text(loc1 - 0.01, loc2 - 0.01, lab, fontsize=12)
                        )
                self.peaks_scatter.append(
                    scatter_xy_points(self.axes, self.peak_list_locs, c='k', s=50,
                                      zorder=2, marker='x')
                )
        ...
        # Shared list selections use the established green/large-X selection
        # ornament.  Keep the display coordinate locally because physical-2D
        # reference selections do not need the 3D axis-value mapping.
        selected_xy = getattr(self, '_selected_2d_peak_xy', None)
        if selected_xy is not None:
            self.axes.scatter(selected_xy[0], selected_xy[1],
                              c=[[0.3, 0.5, 0.0, 1.0]], s=140, marker='x',
                              zorder=21, linewidth=1.5)
    ...
    def draw_figure(self,scale='y', keepaxes=False):
        ...
        if self._is_pseudo2d_projection_case():
            ...
            self.axes3.set_xlim(x_max,x_min)
            if(self.cb_grid.GetValue()==1): #if plotting peaks
                # Legacy conn_data-based 4D marker overlay quarantined in Stage 148D.
                pass
    ...
    def on_key_2d(self, event):
        """Handle current Projection display shortcuts.

        Historical projection-spectrum line-fitting shortcuts (n/r/enter) were
        quarantined after PeakFit became the canonical fitting workflow.
        """
        if event.key == 'h':
            print('h clicked')
    ...
    # if(self.cb_grid.GetValue()==1): #if plotting peaks
        #     Historical connectivity overlay removed in Stage 148D.
        #     for cn in []:
        #             loc1=float(cn.f1)
        #             loc2=float(cn.f2)
        #             lab=cn.p1
    ...
    def on_cb_grid(self, event):
        if self._is_true_2d_spectrum() and event.GetEventObject() == self.cb_grid:
            # Redraw through draw_2d so Peaks is a durable view state and the
            # overlay survives contour/decon/resize redraws.  Preserve zoom.
