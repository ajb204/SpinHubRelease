...
void Catia::FreePrecess_Aph_7(ublas::matrix<double>& G,Dataset& dset, int Atom){
  /*
    NameOfBasis: Aph_7

    The basis is:
    { E,
      [ 2IzSx ,2IzSy, 2IzSz ] x { Site A, Site B }
    }
  ...
  */
  ...
  if(Nucl=="N"){
    ...
    std::cerr<<" Functions: FreePrecess_Aph_7()\n";
    Abort(1);
  };
  ...
  RequiredParam.push_back("R0aph"+marker);
  ...
  for(unsigned int i=0;i<RequiredParam.size();i++){
    if(!(BResolveParam(LocalParam[Atom],RequiredParam[i]))){
      ...
      std::cerr<<" Function FreePrecess_Aph_7()\n"<<std::endl;
      Abort(1);
    };
  };
  double R1aph=ResolveParam(LocalParam[Atom],"R1aph"+marker);
  double R0aph=ResolveParam(LocalParam[Atom],"R0aph"+marker);
  ...
  G(2,2) = R0aph;
  G(3,3) = R1aph;
  G(3,0) = -(1-pb)*(R1aph)*gammaS/(gammaI*2.);
  //
  // Omega
  G(1,2) =  (Omega);
  G(2,1) = -(Omega);
  ...
  G(5,5) = R0aph;
  G(6,6) = R1aph;
  G(6,0) = -pb*(R1aph)*gammaS/(gammaI*2.);
  //
  G(4,5) =  (Omega+DeltaO);
  G(5,4) = -(Omega+DeltaO);
  //
