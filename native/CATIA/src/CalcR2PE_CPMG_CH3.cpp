/*
 ...
 based on HtoC_CH3_exchange_800_DC_dfh_v2.c

 Written by Flemming on September 29 2008
 */
...
void Catia::CalcR2PE_CPMG_CH3(Dataset& dset, int GlobalAtom) {
	//
	...
	double taub = atof(dset.vpar("taub")[1].c_str());
	for (unsigned int i = 0; i < dset.ncyc[LocalAtom].size(); i++) {
	...
	taucp.push_back(taub);

	std::string select_flg = to_lower_copy(dset.vpar("select_flg")[1]);
	std::string phase_flg = to_lower_copy(dset.vpar("phase_flg")[1]);

	ublas::matrix<double> p90x, p90mx, p90y, p90my, Hinv;
	...
	CalcMatrix(Hinv, dset, GlobalAtom, "Iinv", 0.);
	...
	for (unsigned int p = 0; p < 4; p++) {
		...
		//
		//Palmer elements - START
		propagate(Gfree[dset.ncyc[LocalAtom].size() + 2], Sigma);
		if (p == 0 || p == 2) { //pi x
			if (phase_flg == "x") {
				propagate(p90x, Sigma);
				...
				std::cerr << " phase_flg must be either 'x' or 'y'\n";
				std::cerr << " Function .CalcR2PE_CPMG_CH3();" << std::endl;
				Abort(1);
			}
		} else {
			if (phase_flg == "x") {
