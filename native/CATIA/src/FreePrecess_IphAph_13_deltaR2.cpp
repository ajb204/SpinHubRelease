...
void Catia::FreePrecess_IphAph_13_deltaR2(ublas::matrix<double>& G, Dataset& dset, int Atom) {
	/*
	...
	for (unsigned int i = 0; i < 2; i++) {//when i=0, we are in ground state. when i=1. we are in excited state
		// Auto Relaxations
		G(1 + i * 6, 1 + i * 6) = R0iph -i * R0iph + i * (R0iph-dBB*dR0taume)*dR0;
		G(2 + i * 6, 2 + i * 6) = R0iph -i * R0iph + i * (R0iph-dBB*dR0taume)*dR0;
		G(3 + i * 6, 3 + i * 6) = R1iph;
		G(3 + i * 6, 0) = -(1 - pb + i * (2 * pb - 1)) * R1iph * gammaS / (_gammaH * 2.);
		G(4 + i * 6, 4 + i * 6) = R0iph -i * R0iph + i * (R0iph-dBB*dR0taume)*dR0;
		G(5 + i * 6, 5 + i * 6) = R0iph -i * R0iph + i * (R0iph-dBB*dR0taume)*dR0;
		G(6 + i * 6, 6 + i * 6) = R1aph;
