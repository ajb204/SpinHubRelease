...
void Catia::CalcR2CW_3st_CPMG(Dataset& dset, int GlobalAtom) {
	...
	for (unsigned int p1 = 0; p1 < 2; p1++) {
		for (unsigned int p2 = 0; p2 < 2; p2++) {
			CalcCoherence_3st("Sz", Sigma, dset); //the dset contains the basis information
			if (p1 == 0) {
			...
			Intensity[2] += Detect("Sz", 2, dset, Sigma) * (p1 - 0.5) * 2.;
		}
	}
	//
	double InitMag = MajorPeakIntensity3States(Pb_3st(dset), Pc_3st(dset), Kex_ab_3st(dset), Kex_ac_3st(dset), Kex_bc_3st(dset), (dset._gamma / _gammaH) * dset._sfrq * 2 * DFH_PI
			* DeltaOmega_ab(dset, GlobalAtom), (dset._gamma / _gammaH) * dset._sfrq * 2 * DFH_PI * DeltaOmega_ac(dset, GlobalAtom), Intensity[0] / 4.0, Intensity[1] / 4.0,
			Intensity[2] / 4.0);
	//
	for (unsigned int ncycC = 0; ncycC < dset.ncyc[LocalAtom].size(); ncycC++) {
		...
		dset.R2_calc[LocalAtom][ncycC] = -log(MajorPeakIntensity3States(Pb_3st(dset), Pc_3st(dset), Kex_ab_3st(dset), Kex_ac_3st(dset), Kex_bc_3st(dset), (dset._gamma / _gammaH)
				* dset._sfrq * 2 * DFH_PI * DeltaOmega_ab(dset, GlobalAtom), (dset._gamma / _gammaH) * dset._sfrq * 2 * DFH_PI * DeltaOmega_ac(dset, GlobalAtom), Intensity[0]
				/ 4.0, Intensity[1] / 4.0, Intensity[2] / 4.0) / InitMag) / time_T2;
