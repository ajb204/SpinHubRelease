...
sys.path.append(os.path.join(Path(__file__).parent.absolute(),'../integrated_software/fuda_py3/py'))
#os.environ["PATH"] += os.pathsep + os.path.join(Path(__file__).parent.absolute(),'../bin')

print(os.environ["PATH"])


import fudaIO, dataIO,string
from fuda import *
...
print (sys.argv)
ProgramBaseName=(sys.argv[0].split('/'))
...
if (len(sys.argv) < 3): 
    ...
    sys.stderr.write("                                           flemming@pound.med.utoronto.ca\n")
    sys.exit(2)
elif(len(sys.argv)==4):
    #ProgramBaseName=(string.split(sys.argv[0],'/'))
    #ProgramBaseName=ProgramBaseName[len(ProgramBaseName)-1]
    PARAMETERFILE=['unidecNMR','unidecParfile']
...
def read_procpar_param(procparfile,param,type):
    if(type[0].lower()=='v'):
        ...
        return read_procpar_param_unidec(procparfile)
    else:
...
def read_procpar_param_unidec(procparfile):
    out=[]
    fudaIO.get_size(fd,2)
...
def read_input_line(line):
    param=(line.strip().split('=')[0]).upper()
    #
    # Did we find an '=' sign
...
for line in inputfile.readlines():
    if ( not line[0] == "#") and ( len(line) > 2 ) : 
        ...
        if (par == "SPECFILE"):
            specfile=value.strip()            
        elif (par == "NOISE"):
            noise=float(value)
        elif (par == "PEAKLIST"):
            peaklistfile=value.strip()
        elif (par == "ZCOOR"):
            zcoor=value.strip()
        elif (par == "PARAMETERFILE" ):
            parameterfile=[]
            value=((value.split('('))[1].split(')'))[0]
            for its in value.split(';'):
            ...
            FITBIEXP=value.strip()
        elif ( par == "FIT180EXP" ):
            FIT180EXP=value.strip()
        elif ( par == "SHAPE" ):
            shape=value.strip()
        elif ( par == "DELAYFACTOR" ):
            delayfactor=float(value)
        elif ( par == "ISOTOPESHIFT" ):
            if ( value.strip() == "N" or value.strip() == "n" ):
                isotopeshift='n'
            else:
                value=value.split('(')[1].split(')')[0]
                isotopeshift=[]
                for numbers in value.split(','):
                    if ( numbers == "FIX" ):
        ...
        #
            ...
            #
            found_name=0
            for not_def_line in value.split(';'):
                ...
                if (par == "NAME"):
                    ...
                    if ( ( val.strip() == "Y" ) or (val.strip() == "y")) :
                        not_def_restoreparameters=1
                    else:
                        not_def_restoreparameters=val.strip()
                else:
            ...
            #
            for overlap_line in value.split(';'):
...
first=[]
last=[]
outy_tcsh = open('showinghdr.csh', 'w')
outy_tcsh.write('showhdr -verb %s | awk \'{ if ( $2=="FIRST:" || $2=="LAST:" || $1=="OBS" ) { print $3,$4,$5 }} \' > templine' % (specfile,))
outy_tcsh.close()
os.system('tcsh showinghdr.csh')

tempfile=open('templine','r')
#
items=tempfile.readline().split()
if not zcoor=="3D":
    if zcoor=="2D":
...
if ( not ( zcoor == "2D" or zcoor =="3D" ) ): 
    #find directoryname:
    if ( specfile[0] == '/' ):
        tree=specfile[1:len(specfile)].strip().split('/')
        dirname='/'
        for level in range(len(tree)-1):
            dirname=dirname+tree[level]+'/'
    else:
        dirname=""
        tree=specfile.strip().split('/')
        for level in range(len(tree)-1):
            dirname=dirname+tree[level]+'/'
    #
...
#
try:
    peakfile=open(peaklistfile.strip(),'r')
except (IOError):
...
for line in peakfile.readlines():
    ...
    if ( line.split()[0] == "Assignment"):
        continue
    if ( line.split()[1] == "peaks" ):
        continue
    # The peakline structure is:
    # name,omega1,omega2,omega3,FIT_FLAG,OVERLAP_FLAG
    # If FIT_FLAG=0     : No fit has been preformed
    # If FIT_FLAG=1     : The fit has been preformed and converged
    # if FIT_FLAG=-1    : The fit has been preformed and did not converge
    ...
    if ( len(FILESIZE) == 2 or len(FILESIZE) == 3 ):
        if ( zcoor == "3D" ):
            ...
            #
            if ( float(linetosave[1]) > first[0] ):
                linetosave[1]=float(linetosave[1])-(first[0]-last[0])
            if ( float(linetosave[1]) < last[0] ):
                linetosave[1]=float(linetosave[1])+(first[0]-last[0])
    ...
    if (peakline[j][4] == 0):
        ...
        if ( verboselevel > 2 ):
            ...
            for i in range(len(peak_fit_array)):
                print("\t",peak_fit_array[i]['isotopeshift'], end=' ')
            sys.stdout.write("\n")
        NoSlices = 0
        if ( zcoor == "2D" ):
            NoSlices = 1
        ...
        if ( printdata=='y' or printdata=='Y'):
            ...
            if ( lm_get('fit_converged') ):
                ...
                eval_data_recalc()
                if ( zcoor=="3D" ):
                    dataout.write('%11s %11s %11s %11s %11s \n' % ('# F3(ppm)','F2(ppm)','F1(ppm)','Data','Calc'))
                else:
                    dataout.write('%11s %11s %11s %11s \n' % ('# F2(ppm)','F1(ppm)','Data','Calc'))
            else:
                if ( zcoor=="3D"):
            ...
            for slice in range(NoSlices):                
                if not ( zcoor=="3D" ):
                    for f1f2 in range(int(ndata/NoSlices)):
                        #
                        ...
                        if ( lm_get('fit_converged') or ( 1==1) ):
                            dataout.write('%11.4e %11.4e %11.4e %11.4e \n' % ( \
                                eval_get_data(int(f1f2+slice*ndata/NoSlices))[0], \
                                eval_get_data(int(f1f2+slice*ndata/NoSlices))[1], \
                                SpecData[int(f1f2+slice*ndata/NoSlices)], \
                                eval_get_data(int(f1f2+slice*ndata/NoSlices))[NDIM] \
                                ))
                        else:
                        ...
                        if ( f1f2+slice*ndata/NoSlices+1 < ndata ):
                            if ( math.fabs(\
                               eval_get_data(int(f1f2+slice*ndata/NoSlices))[1]- \
                               eval_get_data(int(f1f2+slice*ndata/NoSlices+1))[1] \
                               ) > 1e-10 ):
                                dataout.write('  \n')
                    dataout.write('  \n   \n')
            if not (zcoor=="3D" ):
                ...
                if ( dumpparameters ):
                   ...
                   for i in range(NoParameters[k]):
                        parfile.write("%s %20.13e\n" % (\
                            specparam[i+SpecStart][0].split('_')[0],\
                ...
                if ( write_individual ):
                    ...
                    for i in range(NoParameters[k]):
                        ...
                        if ( prename == "j" or prename == "w" ):
                            prename=specparam[i+SpecStart][0].split('_')[0]+"(Hz)"
                            ...
                            prename=specparam[i+SpecStart][0].split('_')[0]+"(ppm)"
                            val=specparam[i+SpecStart][1]
                            esd=specparam[i+SpecStart][2]
                        else:
                            prename=specparam[i+SpecStart][0].split('_')[0]
                            val=specparam[i+SpecStart][1]
                            esd=specparam[i+SpecStart][2]
...
gpversion=open('templine','r').readline().split()
    ...
    if ( gpversion[0] == "gnuplot" ):
        if ( float(gpversion[1][0:3]) > 3.7 ):
            if gpversion[1][3:4] > 'b' :
                ...
                gppointer=open('templine','r').readline().split()
                sys.stderr.write("\n\n *******************************************************************\n")
